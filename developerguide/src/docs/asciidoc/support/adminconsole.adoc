...
[[tools_recyclebin]]
==== RecycleBin
削除されたEntityデータを確認し、元に戻したり完全に削除したりすることができます。

===== Entity一覧
RecycleBinを選択すると、Entity定義の一覧が表示されます。
一覧から対象のEntityを選択し、行をダブルクリックすると、そのEntityの削除データ一覧を表示できます。

`Purge target date` には画面表示時に当日の日付が自動的に設定されます。
日付を指定すると、その日付より前に削除されたデータが処理対象になります。

`Get Data Count` を有効にすると、各Entityの削除データ件数を取得して一覧に表示します。
データ件数の取得には時間がかかる場合があるため、必要な場合にのみ有効にしてください。

Entityを選択して `Clean` をクリックすると、指定した日付より前に削除されたデータをEntity単位で完全に削除します。
Cleanはサーバ側で非同期に実行され、処理状況や結果はメッセージ欄に表示されます。
`Refresh` をクリックすると、Entity一覧を再取得します。

===== 削除データ一覧
Entity一覧で対象Entityの行をダブルクリックすると、削除データ一覧が表示されます。
一覧には、削除データの `Name` と `Deleted Date` が表示されます。
Entity一覧で指定した `Purge target date` より前に削除されたデータが表示対象となります。

削除データを選択して `Restore` をクリックすると、選択したデータを元に戻します。

削除データを選択して `Clean` をクリックすると、選択したデータをRecycleBinから完全に削除します。
完全に削除したデータは元に戻せないため、対象を確認してから実行してください。

`Refresh` をクリックすると削除データ一覧を再取得し、 `Back` をクリックするとEntity一覧に戻ります。
