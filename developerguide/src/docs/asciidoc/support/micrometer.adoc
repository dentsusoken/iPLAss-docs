...
==== 標準メトリクス
NOTE: 標準メトリクスの収集には、Micrometerのcoreモジュールを使用しています。iPLAssで利用しているMicrometer(coreモジュール)のバージョンは、 `1.17.0` です。 +
...
**** `operation_name`：S3操作名（GetObject, PutObject, HeadObject, GetObjectTagging, DeleteObject）
