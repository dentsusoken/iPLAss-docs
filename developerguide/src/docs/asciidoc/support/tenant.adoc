...
それぞれの利用停止範囲は、ログインユーザー情報やアクセスパス（対象Action、対象WebApi、対象Entity、対象機能タイプ）を参照して、GroovyScriptで制御できます。
...
Feature Type, Feature Available Condition Script, Feature Unavailable Customize Message の条件編集領域は、以下のボタンを押下することで制御できます。

* `Add Feature Condition` ボタンをクリックして条件編集領域を追加します。
* `Remove Feature Condition` ボタンをクリックして条件編集領域を削除します。
|Feature Available Condition Script
|特定の機能タイプに対する利用可否をGroovyScript形式で定義します。未指定の場合、全てが利用不可となります。 +
対象の機能タイプのメタデータがバインドされるので、それをもとにtrue(利用可)/false(利用不可)を返すように定義します。 +
利用可能なバインド変数は、編集ダイアログの「Notes」を参照してください。
|Feature Unavailable Customize Message
|特定の機能タイプが利用不可の場合に発生する例外に対してメッセージをGroovyTemplate形式で指定できます。
|===
