...
. JasperReports(PDF)、JXLS(Excel/PDF)、またはApache POI(Excel)用のテンプレートファイルを作成しておきます。
...
PDF(JXLS) ※JXLSテンプレートで生成したExcel(XLSX)を、PDF変換サービスでPDFに変換して出力します。変換環境の詳細は、<<Report-PdfConversionService,後述の「PDF変換サービス（Gotenberg）」>>を参照してください。
...
※PDF出力は、JasperReports形式（PDF(Jasper)）またはJXLS形式（PDF(JXLS)）で対応しています。PDF(JXLS)を利用する場合、PDF変換サービス（変換環境）が必要です。XLS(JXLS)／XLSX(JXLS)出力のみを利用する場合、変換環境は不要です。
|File|
...
※PDF形式においては、Passwordにセットした値はPDFファイルのUser Passwordとして設定されます。また、Passwordのみセットし、後述のOwner Passwordをセットしていない場合、PDFファイルのOwner PasswordもPasswordにセットした値に設定されます。

※PDF(JXLS)形式においても、JasperReports形式のPDFと同様にパスワードが設定されます。暗号化（AES-256）はPDF変換後にiPLAss（JVM）内で実施されるため、パスワードがPDF変換サービスへ送信されることはありません。パスワードとオーナーパスワードのいずれも設定しない場合、暗号化されないPDFとして出力されます。

※パスワードは、いずれのReportTypeにおいてもXLS形式には対応していません。Excelに対してパスワード指定する場合はXLSX形式を利用してください。

|Owner Password Attribute Name|
出力するPDF形式のレポートに対してOwner Passwordを設定する場合に、そのパスワード値をセットしたRequestのAttribute名を指定します。

設定方法は `Password Attribute Name` と同様です。

※Owner Passwordのみをセットし、Passwordをセットしていない場合、PDFファイルのOwner Passwordのみ設定され、User Passwordは設定されません。

※JXLS形式（PDF(JXLS)出力）においても利用可能です。オーナーパスワードは、この項目で指定したAttribute名からRequestのAttributeとして取得します。
...
[[Report-PdfConversionService]]
=== PDF変換サービス（Gotenberg）
PDF(JXLS)形式の出力では、JXLSテンプレートから生成したExcel(XLSX)をPDF変換サービスへ送信し、PDFに変換した結果を出力します。
ExcelからPDFへの変換処理はiPLAss（JVM）の外部で実行されるため、変換の負荷に応じて、変換サービスを単独でスケールアウトすることが可能です。
なお、XLS(JXLS)／XLSX(JXLS)の出力のみを利用する場合、PDF変換サービスは不要です。

PDF変換サービスとしては、HTTP APIを提供するOSSのドキュメント変換サービスである link:https://gotenberg.dev[Gotenberg^] を利用します。
Gotenbergはコンテナイメージとして配布されており、iPLAssでは8.xでの動作を検証しています。

パスワードによる暗号化は、変換後にiPLAss（JVM）内で実施されるため、パスワードが変換サービスへ送信されることはありません。
パスワードの設定については、「<<Report-Settings,設定>>」を参照してください。

実行環境に応じて、以下のような構成で利用することができます。

* ローカル環境（Docker）
+
開発環境などでは、Dockerコンテナとして起動して利用します。docker composeへの組込みも可能です。
+
----
docker run --rm -p 3000:3000 gotenberg/gotenberg:8
----
+
NOTE: 上記コマンド例の `8` はメジャーバージョンのtagであるため、Gotenbergのリリース時に同一tagのままでイメージの内容が更新されます。開発での利用を想定した例です。本番環境で利用する場合は、動作確認済みのバージョンの固定tag（ `gotenberg/gotenberg:8.x.y` の形式）を指定してください。

* Kubernetes／Amazon ECS
+
レプリカを増やすことでスケールアウトすることができます。

* Google Cloud Run／AWS App Runner
+
マネージド環境で、リクエストに応じた自動的なスケールアウトを利用することができます。

WARNING: Gotenbergは認証機構を持たないため、インターネットへ直接公開しないでください。iPLAssからアクセス可能なプライベートネットワーク上に配置してください。

WARNING: 変換対象のファイルは、PDF変換サービスへ送信されます。データの機密度を考慮した配置を行ってください。

NOTE: Gotenbergのコンテナイメージのバージョンはtagで固定してください。バージョンを変更した場合には、帳票レイアウトを再確認してください。

NOTE: 変換サービスが障害などで利用できない場合でも、失敗するのはPDF(JXLS)形式の出力のみです。他の出力タイプ（XLS／XLSX、JasperReportsのPDFなど）への影響はありません。

NOTE: 変換サービスの稼働状況を確認できるよう、ヘルスチェックなどの監視の仕組みへの組込みを推奨します。

変換サービスの接続先（baseUrl）などのservice-config（PdfConversionService）の設定の詳細は、<<../../serviceconfig/index.adoc#PdfConversionService,PdfConversionService>>を参照してください。
