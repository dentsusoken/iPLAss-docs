...
出力するレポートに対して閲覧パスワードを設定する場合に、そのパスワード値をセットしたRequestのAttribute名を指定します。
...
NOTE: `Password Attribute Name`を設定し、`Owner Password Attribute Name`を設定していない場合、編集パスワードも`Password Attribute Name`で指定したAttributeの値に設定されます。

※パスワードは、いずれのReportTypeにおいてもXLS形式には対応していません。Excelに対してパスワード指定する場合はXLSX形式を利用してください。

|Owner Password Attribute Name|
出力するレポートに対して編集パスワードを設定する場合に、そのパスワード値をセットしたRequestのAttribute名を指定します。

設定方法は`Password Attribute Name`と同様です。

NOTE: `Owner Password Attribute Name`を設定し、`Password Attribute Name`を設定していない場合、閲覧パスワードは設定されず編集パスワードのみ設定されます。
