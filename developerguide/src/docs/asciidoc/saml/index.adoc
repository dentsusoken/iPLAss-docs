...
|Enable Unsolicited Response||非要求応答（IdP-initiated SSO）を有効にする場合チェック

CAUTION: IdP-initiated SSOのフローはCSRFなどの攻撃に対する脆弱性が確認されています。特別な要件がない限り当フラグは有効化せずSP-initiated SSOを使用することを強く推奨します。
...
|===
|設定項目|必須|設定値
|Endpoint URL|○|SingleSignOnServiceのエンドポイントURL(Location)
...
[NOTE]
====
IdP側から送信されるSAMLメッセージでPOST Bindingを利用する場合、iPLAss側のセッションID(`JSESSIONID`)CookieのSameSite属性が `None` に設定されている必要があります。これを設定しない場合、クロスサイトリクエストでCookieが欠落し、認証が正常に動作しないことがあります。

* iPLAssの設定項目「Request Binding」（<<sp_ssosetting,SSO Setting>>）は、iPLAss側からIdPへメッセージ送信する際のバインディングであり、IdP側から送信されるSAMLメッセージ（SAML Response等）のバインディングではありません。IdP側からの送信方式はIdP側で指定されます。
* IdP側から送信されるSAMLメッセージの代表がSAML Responseです。その特性上、メッセージ送信方式は既定でPOST Binding方式が採用されていることが多いです。その確認・設定方法については、対象のIdP（Entra ID / Okta など）のドキュメントを参照してください。
====
...
[NOTE]
====
SP側から送信されるSAMLメッセージでPOST Bindingを利用する場合はセッションCookie設定の考慮が必要です。前述の「iPLAssをSPとして動作させる」のNOTEと同様となるため、そちらを参照してください。
SP側の設定の確認方法は、対象のSPのドキュメントを参照してください。
====
