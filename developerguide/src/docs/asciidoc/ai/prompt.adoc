...
** Amazon Bedrock, Amazon Bedrock AgentCore サービスを利用することができます。
...
include::prompt_setting_platform_knownarrator.adoc[]
...
[TIP]
====
開発環境等で外部のAIサービスを利用できない場合は、スタブ用のサービスを利用して固定のレスポンスを返すことができます。 +
スタブ用のサービスを利用する場合は、link:../../serviceconfig/index.html#AiPromptService_AiPromptServiceStub[Service-Config AiPromptServiceStub^]を参照し、
Service-Config を設定してください。
====
