...
|連動プロパティ設定
|Select
|他のプロパティと連動して選択可能項目をフィルターすることができます。
詳細は<<taskpropertyeditorlinkproperty, 連動プロパティの設定>>を参照してください。
...
[[taskpropertyeditorlinkproperty]]
.連動プロパティの設定
連動プロパティを設定することで、他のプロパティ選択値と連動して選択可能な項目をフィルターすることができます。

.フィルター条件
Editorの表示タイプが `Select` の場合のみ動作します。
上位の選択値（連動元）に合わせて選択可能な項目を絞り込みます。

image::parts_tasklist_linkproperty-searchview.png[]
...
|連動元プロパティ名
|連動元になるプロパティを指定します。 +
連動元プロパティに指定可能なプロパティは以下です。

====
taskStatus(Status):: タスクステータス
taskPriority(Priority):: タスク優先度
====

NOTE: 連動元プロパティ名と連動元カスタムプロパティ名はどちらか片方のみ設定できます。

|連動元カスタムプロパティ名
|連動元になるカスタムプロパティを指定します。 +
ユーザータスクカスタムプロパティエンティティのプロパティを指定します。 +
連動元カスタムプロパティに指定可能なプロパティは以下です。

====
Selectプロパティ:: UserTaskListSelectPropertyEditorの `表示タイプ` が `Select` または `Radio` のもの
Referenceプロパティ:: UserTaskListReferencePropertyEditorの `表示タイプ` が `Select` のもの
====
...
+
image::parts_tasklist_setting-linkproperty-main.png[]

* 参照している各カテゴリエンティティ +
Category1 ＞ Category2 ＞ Category3
+
image::parts_tasklist_setting-linkproperty-category.png[]

.フィルター条件設定
. 各Categoryを配置
+
image::parts_tasklist_setting-linkproperty-searchlayout.png[]

. Category1の設定 +
Category1エンティティをセレクトアイテムの選択値に連動させる
+
image::parts_tasklist_setting-linkproperty-searchlayout-category1.png[]

. Category2の設定 +
Category2エンティティをCategory1の選択値に連動させる
+
image::parts_tasklist_setting-linkproperty-searchlayout-category2.png[]

. Category3の設定 +
Category3エンティティをCategory2の選択値に連動させる
+
image::parts_tasklist_setting-linkproperty-searchlayout-category3.png[]
...
image::parts_tasklist_setting-linkproperty-searchcondition1.png[]

`セレクトアイテム` を選択することで、連動先の `カテゴリ1` の選択可能項目が設定される。

image::parts_tasklist_setting-linkproperty-searchcondition2.png[]

`カテゴリ1` を選択することで、連動先の `カテゴリ2` の選択可能項目が設定される。

image::parts_tasklist_setting-linkproperty-searchcondition3.png[]

`カテゴリ2` を選択することで、連動先の `カテゴリ3` の選択可能項目が設定される。

image::parts_tasklist_setting-linkproperty-searchcondition4.png[]

より上位の連動元が未選択になると、
連動先は未選択状態になり、選択可能項目もクリアされる。

image::parts_tasklist_setting-linkproperty-searchcondition5.png[]
...
|タスク詳細カスタムレイアウト表示に関する設定を行います。
詳細は<<taskviewdetaillayout, タスク詳細カスタムレイアウト設定項目>>を参照してください。
|===
...
image::parts_taskview_setting-layout.png[align="left"]
...
ボタン:: カスタムで表示したいボタンを追加します。 詳細は<<taskviewdetaillayout_button, タスク詳細ボタン設定>>を参照してください。
JavaScriptコード:: タスク詳細のScriptタグ内に出力するJavaScriptコードを入力します。
編集でJavaScriptコードを有効化:: チェックした場合、`JavaScriptコード` に設定したJavaScriptが編集画面で有効になります。
詳細でJavaScriptコードを有効化:: チェックした場合、`JavaScriptコード` に設定したJavaScriptが詳細画面で有効になります。
カスタムステータス更新処理クラス名:: ステータス更新時にカスタマイズ処理を行いたい場合は `org.iplass.mtp.view.top.parts.workflow.UpdateAssignStateInterrupter` インターフェースを実装するクラスを指定してください。 詳細は<<taskviewdetaillayout_customstatusupdate, タスク詳細カスタムステータス更新処理>>を参照してください。

|カスタムスクリプトセクション配置エリア
|緑枠の部分はタスク詳細画面の該当部分にカスタマイズを入れるための設定です。
画面右側の `Section` 内の部品をドラッグ&ドロップで配置できます。
定義済みのテンプレートやHTMLベースのスクリプトを組み込むことができます。 +
配置できるカスタムスクリプトセクションについて詳しくは<<taskviewdetaillayout_customscript, タスク詳細カスタムスクリプトセクション設定>>を参照してください。
|===

[[taskviewdetaillayout_button]]
.タスク詳細ボタン設定
タスク詳細画面に追加するカスタムボタンに対する設定を行います。
...
|表示
|チェックした場合、カスタムボタンを表示します。

|表示判定スクリプト
|表示可否を判定するスクリプトを設定します。
詳細は<<taskviewdetaillayout_judgedisplayscript, タスク詳細表示判定スクリプト>>を参照してください。

|表示タイプ
|編集画面、詳細画面のどちらに表示するかを設定できます。
表示タイプが指定されていない場合は、編集画面、詳細画面に表示されます。
...
|表示ラベル
|ボタンに表示するラベルを設定します。カスタマイズ多言語対応可能です。
...
詳細は<<taskviewdetaillayout_buttoncustomstyle, ユーザタスク詳細ボタンカスタムスタイル>>を参照してください。
|===

[[taskviewdetaillayout_judgedisplayscript]]
.タスク詳細表示判定スクリプト
タスク詳細画面のカスタムボタン表示可否を判定するGroovyScriptを設定します。
...
EDIT : 編集画面 +
VIEW : 詳細画面
userTask:: 表示対象のUserTaskエンティティ。エンティティにはoidとversionがバインドされます。
====

[[taskviewdetaillayout_buttoncustomstyle]]
.タスク詳細ボタンカスタムスタイル
表示されるボタンに対して直接style属性を指定できます。
...
[[taskviewdetaillayout_customstatusupdate]]
.タスク詳細カスタムステータス更新処理
...
====
org.iplass.mtp.view.top.parts.workflow.UpdateAssignStateInterrupter
====

[[taskviewdetaillayout_customscript]]
.タスク詳細カスタムスクリプトセクション設定
タスク詳細画面に追加するカスタムスクリプトに対する設定を行います。

[[taskviewdetaillayout_customscript_common]]
.共通設定項目
各セクションで共通の設定項目です。
...
[[taskviewdetaillayout_customscript_script]]
.スクリプトセクション
...
[[taskviewdetaillayout_customscript_template]]
.テンプレートセクション

詳細画面や編集画面の出力内容を独自に組み込む場合に設定します。
出力内容は別途テンプレートとして定義されているものを指定します。
スクリプトとの違いは、スクリプトは対象のタスク詳細定義でのみ利用できますが、
テンプレートとして定義することで、GroovyTemplate形式以外のJSP形式での実装や、
他のタスク詳細定義でも利用することが可能になります。
...
.アクション/WebAPI設定
タスク一覧、タスク詳細画面の各種アクション、WebAPIを設定します。
...
|Task List Setting
|タスク一覧のアクション、WebAPIをカスタマイズする際の設定を行います。

Search WebAPI ::
タスク検索時に実行されるWebAPIを設定します。
View screen display action ::
タスク一覧の詳細リンククリックで実行されるアクションを設定します。
Edit screen display action ::
タスク一覧の編集リンククリックで実行されるアクションを設定します。
Assignment WebAPI ::
タスク一覧の割当リンククリックで実行されるWebAPIを設定します。

|Task Detail Setting
|タスク詳細のアクション、WebAPIをカスタマイズする際の設定を行います。

Edit screen display action ::
タスク詳細の編集ボタンクリックで実行されるアクションを設定します。
Cancel action ::
タスク詳細のキャンセルボタンクリックで実行されるアクションを設定します。
Workflow history WebAPI ::
タスク詳細のワークフロー履歴セクション表示時に実行されるWebAPIを設定します。
Status update action ::
タスク詳細の各タスク処理ボタンクリックで実行されるアクションを設定します。
Agency status update action ::
管理可能なタスク詳細の各タスク処理ボタンクリックで実行されるアクションを設定します。
Delegate action ::
タスク詳細の各委譲ボタンクリックで実行されるアクションを設定します。

|Task Detail Dialog Setting
|タスク詳細（ダイアログ表示）のアクション、WebAPIをカスタマイズする際の設定を行います。
