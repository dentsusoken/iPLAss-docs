...
MdcConfigServiceのEntityListing関連の設定については、 <<../../../serviceconfig/index.adoc#MdcConfigService, MdcConfigService>> を参照してください。

[[mdc_entitylisting_relativerange]]
==== MdcRelativeRangeService

フィルター条件の日付相対範囲（RD）や日時相対範囲（RDT）で使用するカスタム相対範囲を定義できます。
カスタム相対範囲はmdc-service-config.xmlの <<../../../serviceconfig/index.adoc#MdcRelativeRangeService, MdcRelativeRangeService>> で設定します。
...
import org.iplass.mtp.impl.mdc.relativeRange.MdcRelativeRangeConverter;
...
PropertyServiceのEntityListing関連の設定については、 <<../../../serviceconfig/index.adoc#PropertyService, PropertyService>> を参照してください。
