...
|アップロードダイアログ
|検索結果部からファイルアップロードを実行する際のダイアログを置き換えるカスタムコンポーネントを設定します。 +
`MIFileUploadDialog` コンポーネントを置き換えます。

|アップロードダイアログ結果
|非同期アップロードの進捗を表示するダイアログを置き換えるカスタムコンポーネントを設定します。 +
`MIFileUploadDialogResult` コンポーネントを置き換えます。
...
mdcProxy:: MSearchViewコンポーネントインスタンスへのプロキシオブジェクト
====
...
|検索結果の表示タイプ
|検索結果の表示タイプを設定します。
未指定の場合は、<<../../../serviceconfig/index.adoc#MdcConfigService, MdcConfigService>> で定義される `searchResultType` が適用されます。
...
- mdcProxy : MSearchView / MIFullTextSearchDialog / MSelectDialogコンポーネントインスタンスへのプロキシオブジェクト
- mdcComponent : MIFullTextSearchDialog / MSelectDialogコンポーネントが `defineExpose` で公開しているオブジェクト
====
...
|自動補完
|[.eeonly]#検索条件に対するAI自動補完の設定を行います。# +
詳細は<<searchcondition_ai_autocomplete, AI自動補完設定>>を参照してください。
|===
...
[[searchcondition_ai_autocomplete]]
.AI自動補完設定
[.eeonly]#通常検索条件の入力時に、AIを利用して関連するプロパティの値をAI自動補完する機能です。#

連動元プロパティのフォーカスアウト時に、設定されたすべての連動元プロパティの値が入力済みであれば、サーバー側でAIプロンプトを実行し、対象プロパティの値を自動的に設定します。
構造化出力を利用するかどうかは、指定したAIプロンプト定義のJSON Schema設定有無で自動判定されます。
...
|AI自動補完用プロンプト
|AI自動補完で使用するAIプロンプト定義名を設定します。
AIプロンプト定義に設定されたプロンプトテンプレートに基づいて補完値が生成されます。

|連動元プロパティ
|AI自動補完の実行トリガーとなるプロパティを設定します。
設定されたすべての連動元プロパティに値が入力された状態で、いずれかの連動元プロパティからフォーカスアウトするとAI自動補完が実行されます。

|反映対象データキー
|指定したAIプロンプト定義にJSON Schemaが設定されている場合に、AI応答のJSONから補完値を取得するためのキーパスを設定します。
ドット区切りでネストされたキーを指定できます。（例：`data.value`）
|===

.AI自動補完の動作
* 連動元プロパティのフォーカスアウトをきっかけにAI自動補完が実行されます。
* 画面内で他のプロパティに対するAI自動補完リクエストが実行中の場合、新たなリクエストはスキップされます。
* AI自動補完実行前にバリデーションチェックが行われ、入力エラーがある場合は実行されません。
* AI自動補完のエラーメッセージは一定時間経過後に自動的に消去されます。
* 通常検索タブ以外のタブ（詳細検索、固定検索）に切り替えた場合、AI自動補完のエラーメッセージはクリアされます。

[NOTE]
====
AI自動補完を利用するには、事前にAIプロンプト定義を作成しておく必要があります。
====
...
詳細は<<searchlayout_reference_property_editor_link_property_config, 連動プロパティの設定>>を参照してください。

|選択フィルター設定
|SELECTFILTER
|選択フィルターの設定を行います。
詳細は<<searchview_propertyeditor_select_filter_config, 選択フィルター設定>>を参照してください。

|参照コンボ設定
|REFCOMBO
|参照コンボの設定を行います。
詳細は<<searchview_propertyeditor_reference_combo_config, 参照コンボ設定>>を参照してください。
...
====
BooleanProperty:: プロパティエディタ設定の `表示タイプ` が `Radio` 、 `CheckBox` 、 `Select` 
...
SelectProperty:: プロパティエディタ設定の `表示タイプ` が `Radio` 、 `CheckBox` 、 `Select` 
...
<<detaillayout.adoc#detaillayout_reference_property_editor_link_property_config_example, 設定例>>を参照してください。

[[searchview_propertyeditor_select_filter_config]]
===== 選択フィルター設定
...
[[searchview_propertyeditor_reference_combo_config]]
===== 参照コンボ設定
...
image::images/operation_searchview_condition_reference_combo.png[]

image::images/operation_searchview_condition_reference_combo_select.png[]
...
+
image::images/operation_searchview_condition_reference_combo_basecategory.png[]

* 参照している各カテゴリエンティティ +
Category1 ＞ Category2 ＞ Category3
+
image::images/operation_searchview_condition_reference_combo_properties.png[]

.検索条件設定
. Category3エンティティを配置
+
image::images/operation_searchview_condition_reference_combo_layout.png[]

. Category3の設定 +
Category3エンティティから上位のCategory2エンティティ、さらに上位のCategory1エンティティの設定を行う
+
image::images/operation_searchview_condition_reference_combo_setting1.png[]
+
image::images/operation_searchview_condition_reference_combo_setting2.png[]
