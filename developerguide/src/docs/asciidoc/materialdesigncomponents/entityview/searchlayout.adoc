...
詳細は<<searchcondition_ai_autocomplete, 自動補完設定>>を参照してください。
|===
...
.自動補完設定
[.eeonly]#通常検索条件の入力時に、AIを利用して関連するプロパティの値を自動補完する機能です。#
...
|連動元プロパティ
|自動補完の実行トリガーとなるプロパティを設定します。
設定されたすべての連動元プロパティに値が入力された状態で、いずれかの連動元プロパティからフォーカスアウトすると自動補完が実行されます。
...
.AI自動補完の動作
* 連動元プロパティのフォーカスアウトをきっかけに自動補完が実行されます。
* 画面内で他のプロパティに対するAI自動補完リクエストが実行中の場合、新たなリクエストはスキップされます。
* 自動補完実行前にバリデーションチェックが行われ、入力エラーがある場合は実行されません。
