...
mdcProxy:: MDetailViewコンポーネントインスタンスへのプロキシオブジェクト
mdcComponent:: MDetailViewコンポーネントが `defineExpose` で公開しているオブジェクト
====
...
|ユーザー名をリンク表示
|チェックした場合、操作ユーザー名をリンク表示にします。

|ユーザー情報表示用View定義名
|操作ユーザー名リンククリック時に表示するView定義名を設定します。
...
|AIチャット入力補完設定
|[.eeonly]#編集画面でAIチャットを利用したプロパティの入力補完機能を設定します。# +
詳細は<<detaillayout_ai_chat_autocomplete, AIチャット入力補完設定>>を参照してください。
...
|SELECTFILTER
|選択フィルターの設定を行います。
詳細は<<detaillayout_reference_property_editor_select_filter_config, 選択フィルター設定>>を参照してください。

|参照コンボ設定
|REFCOMBO
|参照コンボの設定を行います。
詳細は<<detaillayout_reference_property_editor_reference_combo_config, 参照コンボ設定>>を参照してください。
...
|重複を除く
|チェックした場合、重複したレコードを除いた検索結果を返却します。

|連動プロパティ設定
|エンティティの他のプロパティと連動して選択可能項目をフィルターすることができます。
詳細は<<detaillayout_reference_property_editor_link_property_config, 連動プロパティの設定>>を参照してください。

|===

[[detaillayout_reference_property_editor_link_property_config]]
===== 連動プロパティ設定
連動プロパティを設定することで、エンティティの他のプロパティ選択値と連動して選択可能な項目をフィルターすることができます。 +
Editorの表示タイプが `Select` または `CheckBox` のみで動作します。上位に合わせて絞り込みが行われます。

image::images/detailview-linkproperty-overview.png[]
...
|連動元プロパティ名
|連動元になるプロパティを指定します。参照元のエンティティのプロパティを指定します。 +
連動元プロパティに指定可能なのは以下です。

====
BooleanProperty:: プロパティエディタ設定の `表示タイプ` が `Radio` 、 `CheckBox` 、 `Switch` 、 `Select` 
DateProperty:: プロパティエディタ設定の `表示タイプ` が `Text` 
DateTimeProperty:: プロパティエディタ設定の `表示タイプ` が `Text` 
DecimalProperty:: プロパティエディタ設定の `表示タイプ` が `Text` 
FloatProperty:: プロパティエディタ設定の `表示タイプ` が `Text` 
IntegerProperty:: プロパティエディタ設定の `表示タイプ` が `Text` 
LongTextProperty:: プロパティエディタ設定の `表示タイプ` が `Text` 
ReferenceProperty:: プロパティエディタ設定の `表示タイプ` が `CheckBox` 、 `Select` 
SelectProperty:: プロパティエディタ設定の `表示タイプ` が `CheckBox` 、 `Select` 
StringProperty:: プロパティエディタ設定の `表示タイプ` が `Text` 
TimeProperty:: プロパティエディタ設定の `表示タイプ` が `Text` 
====

以下のプロパティは連動元プロパティに設定できません。 +
...
PROPERTY:: 連動元入力（選択）値を `連動絞込み対象プロパティ名` で指定したプロパティに対して条件設定して、選択可能なアイテムを絞り込みます。
EQL::  `連動絞込み対象条件` で設定したEQLを実行し、選択可能なアイテムを絞り込みます。

|連動絞込み対象プロパティ名
|連動元プロパティに対応する参照エンティティ側(自身)のプロパティ名を指定します。連動絞込み対象タイプでPROPERTYを選択した場合に利用されます。 +
連動絞込み対象プロパティ名に設定可能なプロパティは下記のみです。 +

* SelectProperty
* ReferenceProperty

|連動絞込み対象条件
|連動絞込み対象タイプでEQLを選択した際に実行する処理です。 GroovyTemplate形式で記述してください。

.利用可能な変数
以下の変数がバインドされています。
====
user:: ユーザー情報（org.iplass.mtp.impl.auth.UserBinding）
value:: 連動元プロパティの値
====

.連動元プロパティ値の型
変数としてバインドされている `value` の型は連動元プロパティタイプによって異なります。 +
下記それぞれの型がバインドされます。
====
BooleanProperty:: `java.lang.Boolean` の `java.util.List` 
DateProperty:: `java.lang.String` （yyyy-MM-dd形式）の `java.util.List` 
DateTimeProperty:: `java.lang.String` （yyyy-MM-ddTHH:mm:ss.SSS形式）の `java.util.List` 
DecimalProperty:: `java.lang.Number` の `java.util.List` 
FloatProperty:: `java.lang.Number` の `java.util.List` 
IntegerProperty:: `java.lang.Number` の `java.util.List` 
LongTextProperty:: `java.lang.String` の `java.util.List` 
ReferenceProperty:: `java.lang.String` （OID）の `java.util.List` 
SelectProperty:: `java.lang.String` の `java.util.List` 
StringProperty:: `java.lang.String` の `java.util.List` 
TimeProperty:: `java.lang.String` （HH:mm:ss形式）の `java.util.List` 
====

|===

[[detaillayout_reference_property_editor_link_property_config_example]]
（例）メインとなるエンティティで３階層の構造をもったカテゴリエンティティを定義した場合の連動方法 +

.エンティティ定義
* メインエンティティ +
各カテゴリエンティティとCategory1エンティティで保持する区分を保持している。
+
image::images/detailview-linkproperty-main.png[]

* 参照している各カテゴリエンティティ +
Category1 ＞ Category2 ＞ Category3
+
image::images/detailview-linkproperty-category.png[]

.詳細、編集画面設定
. 各Categoryを配置
+
image::images/detailview-linkproperty-detaillayout.png[]

. Category1の設定 +
Category1エンティティをセレクトアイテムの選択値に連動させる
+
image::images/detailview-linkproperty-category1.png[]

. Category2の設定 +
Category2エンティティをCategory1の選択値に連動させる
+
image::images/detailview-linkproperty-category2.png[]

. Category3の設定 +
Category3エンティティをCategory2の選択値に連動させる
+
image::images/detailview-linkproperty-category3.png[]
...
image::images/detailview-linkproperty-detailselect1.png[]

`セレクトアイテム` を選択することで、連動先の `カテゴリ1` の選択可能項目が設定される。

image::images/detailview-linkproperty-detailselect2.png[]

`カテゴリ1` を選択することで、連動先の `カテゴリ2` の選択可能項目が設定される。

image::images/detailview-linkproperty-detailselect3.png[]

`カテゴリ2` を選択することで、連動先の `カテゴリ3` の選択可能項目が設定される。

image::images/detailview-linkproperty-detailselect4.png[]

より上位の連動元が未選択になると、
連動先は未選択状態になり、選択可能項目もクリアされる。

image::images/detailview-linkproperty-detailselect5.png[]

[[detaillayout_reference_property_editor_select_filter_config]]
===== 選択フィルター設定
SELECTFILTERタイプに対する設定を行います。
...
|プロパティ
|選択フィルター用のプロパティを指定します。 +
指定したプロパティに入力された値をもとに参照データを絞り込みます。

|検索実行モード
|参照データの絞り込みをサーバー側で行うかどうかを設定します。 +
 `CLIENT` を選択すると画面表示時に対象の参照データを全件取得するため、参照データの件数が多い場合は `REMOTE` を選択するようなことを想定しています。 +
未選択の場合は `CLIENT` が適用されます。
====
REMOTE:: サーバーサイドで選択フィルターの絞り込みを行います
CLIENT:: サーバーアクセスせずに選択フィルターの絞り込みを行います
====

|検索条件
|選択フィルター項目を検索する際の条件に依存しない検索条件を設定します。

|ソートアイテム
|選択フィルター項目をソートする項目を設定します。

|ソート種別
|選択フィルター項目をソートする種別を設定します。

|検索件数
|選択フィルターで1回ごとに取得する検索件数を設定します。指定しない、または 0の場合はデフォルト値が使用されます。0未満を設定した場合は、すべてのデータを取得します。 +
検索実行モードが `REMOTE` の場合に有効になります。

|検索パターン
|検索パターンを設定します。 +
未指定の場合は `PREFIX` が適用されます。
====
PREFIX:: 前方一致
PARTIAL:: 部分一致
POSTFIX:: 後方一致
...
|再検索時の選択状態パターン
|再検索を実行した際に、選択肢の選択状態をどう扱うかを設定します。 +
未指定の場合は `KEEP` が適用されます。
====
KEEP:: 現在の選択を保持します
CLEAR:: 選択をすべてクリアします
====
...
[[detaillayout_reference_property_editor_reference_combo_config]]
===== 参照コンボ設定
REFCOMBOタイプに対する設定を行います。
...
|ビュー定義名
|参照リンク押下で表示するダイアログのView定義名を設定します。未指定時はデフォルトのViewを使用します。

|詳細画面で参照コンボの親階層を表示
|選択したデータから取得可能な全ての親階層を表示します。

|詳細画面の参照リンクで編集を許可
|参照リンク押下時のリンク先詳細画面で編集を許可します。
...
.詳細、編集画面設定
設定内容は検索条件と同様。 ただし `詳細画面で参照コンボの親階層を表示` がチェックされている場合は、階層で表示される。 +

image::images/operation_detailview_condition_reference_combo.png[]
...
[[detaillayout_ai_chat_autocomplete]]
=== AIチャット入力補完設定
[.eeonly]#編集画面でプロパティラベル横にロボットアイコンボタンを表示し、AIとの対話を通じてプロパティの値を補完する機能です。#

本設定は `プロパティエディタ` そのものではなく、プロパティ設定におけるAI補完機能の設定項目です。

ユーザーがAIチャットパネルでプロンプトを選択しメッセージを送信すると、AIが応答を生成します。
応答に反映可能な値が含まれている場合、「この回答を反映」ボタンを押すことでプロパティの値を自動的に設定できます。
反映後は「反映を取消」ボタンで直前の反映を元に戻すことも可能です。
構造化出力を利用するかどうかは、選択したAIプロンプト定義のJSON Schema設定有無で自動判定されます。
...
|表示
|AIチャット入力補完機能を有効にする場合はチェックします。
有効にすると、編集画面で対象プロパティの入力欄横にAIチャットボタンが表示されます。

|プロンプト
|AIチャットで使用するAIプロンプト定義を設定します。
複数のプロンプトを設定した場合、ユーザーはAIチャットパネルのドロップダウンからプロンプトを選択できます。

|データキー
|選択したAIプロンプト定義にJSON Schemaが設定されている場合に、AI応答のJSONからプロパティの反映値を取得するためのキーパスを設定します。
ドット区切りでネストされたキーを指定できます。（例：`data.value`）

|メッセージデータキー
|選択したAIプロンプト定義にJSON Schemaが設定されている場合に、AI応答のJSONからメッセージテキストを取得するためのキーを設定します。（例：`message`）
|===

image::images/detaillayout_ai_chat_autocomplete.png[]

.AIチャット入力補完の動作
* 編集画面の対象プロパティラベルにロボットアイコンボタンが表示されます。（A）
* ボタンを押すと画面右側にAIチャットパネルが開きます。
* ユーザーはプロンプトを選択し（B）、メッセージを送信できます。（C）
* AIの応答に反映可能なデータが含まれている場合、メッセージ下部に「この回答を反映」ボタンが表示されます。（D）
* 反映ボタンを押すと、AIの応答値が対象プロパティに自動設定されます。
* 反映後は「反映を取消」ボタンで元の値に戻すことができます。（E）
* AIからのエラー応答にリトライ可能フラグが設定されている場合、「リトライ」ボタンが表示されます。

[NOTE]
====
* AIチャット入力補完を利用するには、事前にAIプロンプト定義を作成しておく必要があります。
* AIチャットパネルでプロンプトを切り替えると、現在の会話履歴はクリアされます。
* フォームデータ（現在の編集中の値）はAIリクエスト時に自動的にサーバーに送信されるため、AIは入力内容を考慮した応答を生成できます。
====
