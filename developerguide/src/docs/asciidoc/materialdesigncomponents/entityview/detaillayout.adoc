...
Editorの表示タイプが `Select` または `CheckBox` のみ動作します。上位に合わせて絞込みが行われます。
...
下記それぞれの型の `java.util.List` になります。
====
BooleanProperty:: `java.lang.Boolean` 
...
LongTextProperty:: `java.lang.String` 
ReferenceProperty:: `java.lang.String` （OID）
SelectProperty:: `java.lang.String` 
