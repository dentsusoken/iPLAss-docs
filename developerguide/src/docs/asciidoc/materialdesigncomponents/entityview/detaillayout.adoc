...
StringProperty:: プロパティエディタ設定の `表示タイプ` が `Text` 、 `Select` 
...
.連動元プロパティの値の型
変数としてバインドされている `連動元プロパティの値` の型は連動元プロパティタイプによって異なります。 +
連動元プロパティの多重度が1以外の場合は、それぞれの型のjava.util.Listになります。
====
BooleanProperty:: java.lang.Boolean
DateProperty:: java.lang.String（yyyy-MM-dd形式）
DateTimeProperty:: java.lang.Long（ミリ秒）
