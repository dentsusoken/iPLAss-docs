...
|構造化出力を使用
|AIの応答を構造化データ（JSON形式）として解析する場合はチェックします。
チェックした場合、 `データキー` および `メッセージデータキー` の設定が必要です。

|データキー
|構造化出力が有効な場合に、AI応答のJSONからプロパティの反映値を取得するためのキーパスを設定します。
ドット区切りでネストされたキーを指定できます。（例：`data.value`）

|メッセージデータキー
|構造化出力が有効な場合に、AI応答のJSONからメッセージテキストを取得するためのキーを設定します。（例：`message`）
|===
