...
image::images/parts_tasklist_link-property.png[]
...
<<../entityview/index.adoc#detaillayout_reference_property_editor_link_property_config_example, 設定例>>を参照してください。
...
mdcProxy:: MIUserTaskDetailDialogコンポーネントインスタンスへのプロキシオブジェクト
mdcComponent:: MIUserTaskDetailDialogコンポーネントが `defineExpose` で公開しているオブジェクト
====
...
|Task List Setting
|タスク一覧のWebAPIをカスタマイズする際の設定を行います。

Search WebAPI ::
タスク検索時に実行されるWebAPIを設定します。
View screen display WebAPI ::
タスク一覧の詳細/編集リンククリックで実行されるWebAPIを設定します。
Assignment WebAPI ::
タスク一覧の割当リンククリックで実行されるWebAPIを設定します。

|Task Detail Setting
|タスク詳細のWebAPIをカスタマイズする際の設定を行います。

Workflow history WebAPI ::
タスク詳細のワークフロー履歴セクション表示時に実行されるWebAPIを設定します。
Status update WebAPI ::
タスク詳細の各タスク処理ボタンクリックで実行されるWebAPIを設定します。
Agency status update WebAPI ::
管理可能なタスク詳細の各タスク処理ボタンクリックで実行されるWebAPIを設定します。
Delegate WebAPI ::
タスク詳細の各委譲ボタンクリックで実行されるWebAPIを設定します。
|===
...
a|保存リストの一覧画面から直接File ダウンロードを許可するかを設定します（デフォルト：不可） +
可能とした場合は、ポップアップに `ダウンロード` が表示されます。 +
ダウンロード時のファイル形式は、対象となる
<<../aggregation/index.adoc#aggregation_common_setting, Aggregation定義>> や
<<../entitylisting/index.adoc#mdc_entitylisting_defaultsetting, EntityListing定義>> の `File type` 設定に従います。 +
...
指定するClassは `org.iplass.mtp.mdc.view.topview.parts.listing.SavedListInterrupter` を実装する必要があります。
詳細は<<savedlist_customize, カスタム処理の組み込み>>を参照してください。

|Edit Custom ViewScript
|保存リストの保存画面内に埋め込むカスタムコンテンツを設定できます。
`Template Interpret Type` の設定により、HTML、VUE_SFC、VUE_SFC_LIKE形式で定義できます。
詳細な設定内容は<<script, Script>>と同一のため、参照してください。
...
====
org.iplass.mtp.mdc.view.topview.parts.listing.SavedListInterrupter
====
...
import org.iplass.mtp.mdc.view.topview.parts.listing.SavedListFolderPermission;
import org.iplass.mtp.mdc.view.topview.parts.listing.SavedListInterrupter;
import org.iplass.mtp.mdc.view.topview.parts.listing.SavedListPermission;
...
以下のデータがpropsとしてコンポーネントに引き渡されます。
====
contextMap:: コンテキストマップ。リアクティブなMapオブジェクト。 +
コンテキストマップにデータを格納することで、複数のパーツ（コンポーネント）間でリアクティブにデータを共有することが可能です。
* mdcProxy : MITopViewCustomPartsコンポーネントインスタンスへのプロキシオブジェクト
====
