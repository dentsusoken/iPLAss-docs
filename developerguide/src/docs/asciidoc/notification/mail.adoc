...
** 標準の機能では対応できない設定を行いたい場合（例: `personalizations[n].substitutions` の設定など）には固有オプションを利用しますが、デフォルトでは `personalizations`, `content` など既存の本文パラメータと衝突する特定の固有オプション値はサニタイズされ、メール送信リクエストには反映されません。 +
SendGridV3MailService の `canOverwriteBodyParameter` プロパティを `true` に設定すると、これらの固有オプション値はサニタイズされず、固有オプションで指定した値で既存の本文パラメータを上書きできるようになります。 +
