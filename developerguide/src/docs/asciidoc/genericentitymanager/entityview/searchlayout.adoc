...
|検索結果TABLEの高さ自動調節
|テーブル高さの自動調整方法を設定します。「検索結果TABLEの高さ」が 0 の場合のみ設定可能です。

FIT_TO_ROW_COUNT:: 検索結果の表示件数に応じて調節（デフォルト）
FIT_TO_VIEWPORT:: テーブル全体が画面内に表示されるよう調節
...
|列の固定を許可
|検索結果一覧での列の固定を許可する場合はチェックします。
許可した列はデフォルトで固定され、画面上のピン操作で固定範囲の変更・解除ができます。
許可しない列にはピンが表示されません。 +
許可する列が連続していない場合も、先頭から最も右の許可列までが固定されます。
