...
AdminConsoleの編集画面で設定する項目と対応しています。
...
|多言語表示名リスト（localizedDisplayNameList）
|多言語の表示名を指定します。

|概要（description）
|定義の概要を指定します。

|対象Entity（targetEntityName）
|SpreadSheetの対象となるEntityを選択します。

|グリッドオプション（gridOptions）
...
|デフォルト検索条件（defaultPropertyConditionScript）
...
デフォルト検索条件（defaultPropertyConditionScript）は、画面表示時にフィルタ条件エリアへ初期値を設定するためのGroovyスクリプトです。
...
グリッド初期化スクリプト（gridInitializationScript）は、グリッド生成後にクライアント側で実行する初期化JavaScriptを、Groovyテンプレートで記述します。
...
|列ヘッダー表示（showColumnHeader）
|グリッドの列ヘッダーを表示します。

|自動編集開始（autoEdit）
|セルがアクティブになったとき自動で編集モードを開始します。

|セルナビゲーション有効（enableCellNavigation）
|キーボードやアクティブセルの移動を有効にします。

|自動ツールチップ有効（enableAutoTooltip）
|セル内容が省略表示されたとき自動でツールチップを表示します。

|セル内テキスト選択有効（enableTextSelectionOnCells）
...
|行削除許可（allowDeleteRow）
|行の削除を許可します。

|エクスポート許可（allowExport）
|データのエクスポートを許可します。
...
|結合許可（enableMergeCells）
|セルの結合機能を有効にします。

|セル結合静的設定（mergeCellsConfig）
...
結合許可（enableMergeCells）を有効にした場合、セル結合の設定は、 `セルレイアウト` または `Expression（ルールベース）` のいずれかで行います。
...
NOTE: 同値（sameValue）判定では、空の値同士は結合されません。
...
|多言語設定リスト（localizedDisplayLabelList）
|表示ラベルの多言語設定を指定します。
...
|多重度表示モード（multiValueDisplayMode）
|多重度プロパティの表示方法を指定します。 +
...
===== 文字列エディタ（SpreadSheetStringEditor）
...
===== 長文エディタ（SpreadSheetLongTextEditor）
...
===== 整数エディタ（SpreadSheetIntegerEditor）
...
===== 浮動小数点エディタ（SpreadSheetFloatEditor）
...
===== 小数エディタ（SpreadSheetDecimalEditor）
...
===== 日時エディタ（SpreadSheetDateTimeEditor）
...
===== 真偽値エディタ（SpreadSheetBooleanEditor）
...
|真値ラベル多言語設定リスト（localizedTrueLabelList）
|真値ラベルの多言語設定を指定します。

|偽値ラベル（falseLabel）
|偽値に表示するラベルを指定します。多言語対応が可能です。

|偽値ラベル多言語設定リスト（localizedFalseLabelList）
|偽値ラベルの多言語設定を指定します。
|===

[[spreadsheet_editor_select]]
===== 選択エディタ（SpreadSheetSelectEditor）
...
===== 参照エディタ（SpreadSheetReferenceEditor）
...
カスタム処理クラスは `org.iplass.mtp.view.spreadsheet.SpreadSheetRegistrationInterrupter` インターフェースを実装して作成し、基本設定のカスタム登録処理クラス名（interrupterName）にUtility Class名を指定します。
