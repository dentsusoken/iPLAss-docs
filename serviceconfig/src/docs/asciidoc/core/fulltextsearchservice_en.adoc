...
The following HttpRequestInterceptorFactory is provided by default.

- <<aws2_AWSRequestSigningApacheInterceptorFactory>>

[[aws2_AWSRequestSigningApacheInterceptorFactory]]
.AWSRequestSigningApacheInterceptorFactory
Used to sign requests with AWS IAM credentials. Authentication information uses the <<aws2_AWSSetting>> setting. +
iPLAss 3.2 exclusive feature.

Please specify the implementing class of org.iplass.mtp.impl.fulltextsearch.opensearch.awsv2.AWSRequestSigningApacheInterceptorFactory. +
...
<service>
	...
	<property name="restClientFactory" class="org.iplass.mtp.impl.fulltextsearch.opensearch.DefaultRestClientFactory" >
		<property name="openSearchUrl" value="{SCHEME}://{ADDRESS}:{PORT}"/>
		<!-- To use AWS authentication, AWSSetting must also be set. -->
