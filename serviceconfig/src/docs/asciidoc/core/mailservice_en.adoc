...
| retryIntervalMillis | long | The retry interval when the transmission fails.(in millisecond)
| retryCount | int | The number of retry attempts for email transmission to fail.
| canOverwriteBodyParameter | boolean | Whether to allow API-specific body parameters to overwrite conflicting body parameters (such as personalizations, content, etc.) generated from the Mail settings in the SendGrid Web API v3 request. The default value is false. +
The Mail class used in the SendGrid Web API v3 request is SendGridMail. +
SendGridMail has the putBodyParameter and setBodyParameters methods, which can be used to set API-specific parameters.
The added specific parameters are added to the body of the SendGrid Web API v3 request. +
If this parameter is false, then when an added specific parameter has the same key as a body parameter created from the information set in SendGridMail (e.g., personalizations, content, etc.), the original body parameter value is kept and the conflicting specific parameter value is sanitized. +
By setting this parameter to true, in such key conflicts the values of the specific parameters are used as-is (without sanitization), and they overwrite the existing conflicting body parameters.

In cases where you need to embed different strings in the body or subject for each recipient, and the standard settings cannot handle it, you may consider setting this parameter to true.
|===
