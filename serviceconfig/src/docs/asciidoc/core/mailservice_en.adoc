...
Properties extended with iPLAss.
| mail.customheaderMap | String, in format of Map | Specify the values to be set in the email header in Map format. For each entry, set the header name in the nested `<property>` 's. `name` attribute (key) and the header value in its `value` attribute. +
Properties extended with iPLAss.
