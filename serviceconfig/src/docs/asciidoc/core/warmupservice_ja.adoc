...
applicationTask では、テナントに依存せずサーバー単位で実行するタスク（ <<TomcatJasperJspPreCompileWarmupTask>> など ）を設定してください。
...
tenantTaskMap では、テナント単位で実行するタスク（ <<MetaDataWarmupTask>> など ）を設定してください。
|===
