...
In applicationTask, configure tasks that run per server and are independent of the tenant (such as <<TomcatJasperJspPreCompileWarmupTask>>).
...
In tenantTaskMap, configure tasks to be executed per tenant (such as <<MetaDataWarmupTask>>).
|===
...
Pre-read metadata.。 +
This task is executed per tenant. Configure it in the tenantTaskMap property of WarmupService. +
...
This task is executed per server. Configure it in the applicationTask property of WarmupService. +
