...
標準で以下の HttpRequestInterceptorFactory を提供します。
...
classはorg.iplass.mtp.impl.fulltextsearch.opensearch.awsv2.AWSRequestSigningApacheInterceptorFactoryを指定します。
...
<service>
	...
	<property name="restClientFactory" class="org.iplass.mtp.impl.fulltextsearch.opensearch.DefaultRestClientFactory" >
		<property name="openSearchUrl" value="{SCHEME}://{ADDRESS}:{PORT}"/>
		<!-- AWS 認証を利用する場合、AWSSetting の設定も必要。 -->
