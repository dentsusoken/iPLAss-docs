...
| Item | Value | Description
| store | String | The name of the AuthTokenStore to use. If not specified, `default` AuthTokenStore is used.
| type | String | Type. If not set, it will be `TSK`.
...
| backwardTimeStepWindowSize | int | Number of previous time steps to consider when verifying codes. If set to -1 (default), the handler will use the same value as `timeStepWindowSize`, allowing you to configure the backward window independently from the forward window when needed. |
|===
