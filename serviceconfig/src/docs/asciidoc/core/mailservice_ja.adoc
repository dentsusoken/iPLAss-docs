...
| retryIntervalMillis | long | 送信失敗時のリトライ間隔（ミリ秒）。
| retryCount | int | 送信失敗時のリトライ回数。
| canOverwriteBodyParameter | boolean | SendGrid Web API v3のリクエストで、メールの本文パラメータを上書き可能とするか。デフォルト値は false です。 +
...
この際、本パラメータが false の場合、SendGridMail に設定されている情報から作成する本文パラメータ（例: personalizations, content など）が、追加した固有パラメータに存在する場合はサニタイズされます。 +
本パラメータを true に設定することで、特定の固有パラメータをサニタイズせずに優先的に利用します。
