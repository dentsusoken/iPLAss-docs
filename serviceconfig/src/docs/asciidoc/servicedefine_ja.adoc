...
| [.eeonly]#/appconsole-service-config.xml# | appConsoleに関するデフォルト設定です。Enterprise Editionで利用可能です。
| [.eeonly]#/aws-service-config.xml# | awsに関するデフォルト設定です。Enterprise Editionで利用可能です。
...
| ref | String | プロパティがjavaBeansの場合、 <<Bean, bean要素>> で定義されるnameを指定可能です。refの指定は、value、class指定より優先されます。
...
|aws.secretName|SecretsManagerPassphraseSupplierを利用する場合、シークレット名を指定。AWS認証情報として、デフォルトの認証情報プロバイダーチェーンを使用しています。
|===
...
|===
|項目名|設定値
|aws.secretName|シークレット名を指定。AWS認証情報として、デフォルトの認証情報プロバイダーチェーンを使用しています。
|===
