...
| selectFilterSearchPageSize | int | The number of records retrieved per search/page for the reference-type selection filter (SELECTFILTER). If not specified, the default value is 40.
...
| entityListingSearchLimit | int | Number of records per page in EntityListing search results. The default value is 10.
| entityListingFileSupportType | FileSupportType | Default file format for EntityListing file download. Applied when the `File type` of the EntityListing metadata is not specified. Specify from `CSV` `EXCEL` `SPECIFY`. The default value is `CSV`.
| entityListingFileDownloadWithFooter | boolean | Whether to output a footer in EntityListing file download. The default value is false.
| entityListingFileDownloadFooter | String | Footer wording for EntityListing file download. Effective when `entityListingFileDownloadWithFooter` is `true`.
...
<service>
	...
	<!-- Number of records fetched per search/page for the reference type selection filter -->
	...
	<!-- Number of records per page in EntityListing search results -->
	<property name="entityListingSearchLimit" value="10" />

	<!-- Default file format for EntityListing. CSV, EXCEL, SPECIFY -->
	<property name="entityListingFileSupportType" value="CSV" />

	<!-- Whether to output a footer in EntityListing file download -->
	<property name="entityListingFileDownloadWithFooter" value="false"/>

	<!-- Footer wording for EntityListing file download -->
