...
classはorg.iplass.mtp.impl.micrometer.awsv2.AWSHttpSenderBuilderを指定します。 +
iPLAss 3.2 限定の機能である AWSHttpSenderBuilder を利用して org.iplass.mtp.impl.micrometer.awsv2.AWSHttpSender インスタンスを生成します。 +
service-config に適用する場合、builder 属性に設定してください。 +

Amazon OpenSearch Serviceに対応したHttpSenderの実装クラスです。<<aws2_AWSSetting>>でアクセスキー、シークレットキー、デフォルトのクライアント設定が可能です。以下の項目を設定可能です。

[CAUTION]
====
iPLAss 3.2 では service-config の class 属性に org.iplass.mtp.impl.micrometer.awsv2.AWSHttpSender を設定しないでください。
...
<service>
	...
	-->
		...
		-->
		<!-- AWS SDK for Java 2.x を利用する例。org.iplass.mtp.impl.awsv2.AWSSetting に依存します。builder 属性に指定してください。 -->
		<!--
