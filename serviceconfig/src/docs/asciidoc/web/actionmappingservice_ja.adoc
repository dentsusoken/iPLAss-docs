...
classはorg.iplass.wam.impl.consent.ConsentCheckInterceptorを指定します。

利用規約の同意チェックを行うインターセプターです。WAMモジュールを適用した場合にデフォルトで追加されます。
AuthInterceptorの後に配置され、認証済みユーザのリクエストに対して未同意の利用規約が存在する場合に同意画面へリダイレクトします。
同意チェックの有効/無効はPreference（wam/wamPreference）のconsent設定で制御します。

以下の項目を設定可能です。
[cols="1,1,3", options="header"]
|===
| 項目 | 値 | 説明
| excludeActionPatterns | String[]、複数指定可 | 同意チェックを除外するActionパターン（正規表現）。 +
未設定の場合、デフォルトで `wam/auth/.\*`、`wam/signup/.\*`、`wam/consent/.\*`、`wam/info/.\*`、`gem/auth/.\*`、`mdc/auth/.\*` が適用されます。
|===
