...
=== AI Integration
==== [.eeonly]#Generative AI Platform Integration#
Provides integration with external generative AI services such as Amazon Bedrock and Know Narrator.
Prompt definitions, shared system prompts, and dynamic parameter injection via placeholders are managed as metadata, enabling no-code/low-code incorporation of AI capabilities into applications.
A data connectivity feature for delivering entity data to AI services is also provided for RAG use cases.

==== [.eeonly]#MCP Server#
Provides server functionality compliant with MCP (Model Context Protocol).
By defining MCP Tools as metadata, functions on iPLAss can be invoked from MCP clients.
In addition, MCP Tools for browsing and updating metadata (design-time information) and for searching, creating, updating and deleting entity data are provided out of the box, supporting AI-driven development and data operations with AI agents.
...
Provides functionality to manage user consent for content such as terms of service and privacy policies.
