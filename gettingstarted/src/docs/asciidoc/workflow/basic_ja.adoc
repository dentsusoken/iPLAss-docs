...
以下の各設定項目の説明については<<../../developerguide/workflow/index.adoc#ref_usertask_assign_rule,ワークフローのドキュメント>>に記載されていますので、参照してください。
+
...
今回は承認だけ設定します。
詳しい説明は<<../../developerguide/workflow/index#ref_usertask_result_status,ワークフローのドキュメント>>に記載されておりますので、参照してください。
+
