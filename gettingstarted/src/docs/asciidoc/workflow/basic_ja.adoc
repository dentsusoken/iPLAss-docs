...
以下の各設定項目の説明については<<../../developerguide/workflow/index#ref_usertask_assign_rule,こちら>>に記載されていますので、ご参照ください。
+
...
今回は承認だけとなります。
詳しい説明は<<../../developerguide/workflow/index#ref_usertask_result_status,こちら>>に記載されておりますので、ご参照ください。
+
