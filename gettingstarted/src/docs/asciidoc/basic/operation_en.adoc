...
The <<../gettingstarted/index.adoc#tenant_info,Tenant>> created by iPLAss has two applications ready to use, `GEM Module` and `AdminConsole`.
...
Next, go back to TopView of MetaDataSettings and click the edit button for the `SearchResult List` displayed at the bottom right.

image::images/topview_searchresultlistsetting_en.png[]

The `topview` views (view definitions named `topview`) that were added earlier are now available in the `ResultList` View and `Detail Action View`, so please select them.

image::images/topview_searchresultlistdialog-topview_en.png[]

In this state, save TopView and display the Top screen.
...
The top screen displays `topview`, which is the view definition specified in the `ResultList View`.
...
For this exercise, please copy and use the View definition named `topview`.
Please rename the View to `topview3`.
