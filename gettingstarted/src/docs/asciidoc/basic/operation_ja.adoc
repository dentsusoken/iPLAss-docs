...
iPLAss上に作成された <<../gettingstarted/index.adoc#tenant_info,テナント>> には、初期状態で `GEMモジュール` と `AdminConsole` という2つのアプリケーションが利用可能になっています。
...
次はMetaDataSettingsのTopViewに戻って、一番下の右に表示している `SearchResult List` の編集ボタンをクリックして下さい。

image::images/topview_searchresultlistsetting.png[]

`ResultList View` と `Detail Action View` に先ほど追加した view定義である `topview` が追加されているので、それを選択してください。
...
Top画面に表示されているviewは `ResultList View` で指定した `topview` のままです。
...
今回は 先ほど作成したview定義である、`topview` をコピーしてViewを作成していきます。
