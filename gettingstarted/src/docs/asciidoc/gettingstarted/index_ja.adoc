...
... iPLAss テナント情報の入力
+
テナントとは、iPLAssにおけるシステムの分割単位です。組織や環境ごとに"独立した利用領域"を確保するための仕組みです。 +
iPLAssでは、このテナントによって、　・データ　・設定　・ユーザー管理　などを完全に分離して運用することができます。 +
ここでは、iPLAssを利用するために最初のテナントの情報を入力します。 +
+
