...
To store in AWS S3, specify org.iplass.mtp.impl.lob.lobstore.s3.awsv2.S3LobStore
====
+
...
[CAUTION]
====
The AWS SDK for Java 1.x is in maintenance mode and is scheduled to reach end of life in December 2025. +
iPLAss is deprecating the iplass-ee-aws library based on AWS SDK for Java 1.x and recommends migrating to the iplass-ee-aws2 library based on AWS SDK for Java 2.x. +
If you are using the `org.iplass.mtp.impl.aws.lobstore.s3.S3LobStore` class from the iplass-ee-aws library, +
please migrate your configuration to the `org.iplass.mtp.impl.lob.lobstore.s3.awsv2.S3LobStore` class in the iplass-ee-aws2 library. +
The iplass-ee-aws library is scheduled for future removal.
====
+
[source,xml]
.Example 1
...
=== Bundled modules
The installer bundle the following modules.
...
| 
| iplass-ee-aws2
| Bundled in version 4.0.16 and later. +
Versions prior to 4.0.15 bundle iplass-ee-aws.
...
| Bundled in version 4.0.1 and later.
...
. Place the created install setting file under the path of ``%USERPROFILE%(or $HOME)/.iplass/`` and start the server with JVM option:``-Dmtp.install=<install setting file path>``.
. Access iPLAss URL.
. The "in preparation" page will show up, then it will redirect to the iPLAss login page.

[IMPORTANT]
====
When placing the file under ``%USERPROFILE%(or $HOME)/.iplass/``, the file name have to be ``install.properties``.
...
If not specified, `%USERPROFILE%(or $HOME)/.iplass` will be the iPLAss home directory.
...
Usually, we will modify the generated ``iplass-service-config.xml`` under ``%USERPROFILE%(or $HOME)/.iplass/`` after installation, to perform further configurations.
...
If the specified config file was canceled, the ``iplass-service-config.xml`` under ``%USERPROFILE%(or $HOME)/.iplass/`` will be used instead.
