...
指定がない場合は、 `%USERPROFILE%\.iplass` がiPLAssホームディレクトリとなります。
...
NOTE: インストーラによって作成されるiplass-service-config.xmlはプロプロセス処理が有効化されています。iplass-service-config.xmlを編集する際に `$` や `\` を記述する際にはエスケープ処理が必要になります。 +
