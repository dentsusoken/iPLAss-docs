...
多重度が複数と指定されているプロパティ（プリミティブ型のみ。参照プロパティは除く）の場合、配列添え字を指定することも可能です。なお添え字は0始まりです。配列添え字を指定しない場合は検索結果として取得される値は配列型で返却されます。
...
[[array-value]]
==== Array Value
<array value>は配列を表現する要素です。
多重度が複数と定義されているEntityのプロパティに値をセットしたり、検索条件に指定することが可能です。
...
EntitiManagerのupdateAll()メソッドによる一括更新の際のUpdateConditionにArrayValueを利用して多重度が複数のプロパティの配列の値に特定の値を指定することが可能です。
